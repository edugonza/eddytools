...
Usage:
  eddytools.py events <input_db> <output_dir>
  eddytools.py cases <input_db> <output_dir> [--build]
  eddytools.py (-h | --help)
  eddytools.py --version

Options:
  -h --help     Show this screen.
  --version     Show version.
  --build       build event logs from case notions [default: false].
...
def disc_and_build(mm: Path, new_mm: Path, dump_dir: Path):
...
def case_notion_candidates_cached(mm_path, dump_dir, build_logs=False):
    ...
    candidates_path = Path(dump_dir, 'candidates.yaml')
...
if __name__ == '__main__':
    ...
    if arguments['events']:
        ...
        disc_and_build(mm=input_mm, new_mm=output_mm, dump_dir=output_dir)

    elif arguments['cases']:
        input_mm = Path(arguments['<input_db>'])
        output_dir = Path(arguments['<output_dir>'])
        build_logs = arguments['--build']
        case_notion_candidates_cached(mm_path=input_mm, dump_dir=output_dir, build_logs=build_logs)
